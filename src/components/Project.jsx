    ...
    <li className="mb-10">
      ...
      >
        ...
        <div className="col-span-12 lg:col-span-8 xl:col-span-7 z-10">
          <h3 className="font-medium mt-3 xl:mt-0 text-base group-hover:text-aquamarine text-slate-200 text-balance">
            {data?.project_title}
            <LinkArrow />
          </h3>
          <p className="mt-2 text-sm group-hover:text-bone-white break-words">
            {data?.project_description}
          </p>
          <ul className="flex flex-wrap mt-3">
            {data?.project_build_with.map((build, index) => (
              <BuildWith build={build} key={index} />
            ))}
          </ul>
        </div>
      </a>
